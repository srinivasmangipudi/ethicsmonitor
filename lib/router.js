...
TaggedDilemmasController = DilemmasListController.extend({
	//override parent functions
	subscriptions: function() {
		this.dbGemsSub = Meteor.subscribe('dbGems');
		this.userProfileSub = Meteor.subscribe('singleUser', this.myId());

		this.dilemmasSub = Meteor.subscribe('taggedDilemmas', this.params.tagName, this.findOptions());
	},
	dilemmas: function() {
		return Dilemmas.find({tags: {$in: [this.params.tagName]}}, this.findOptions());
	},
	sort: {votes: -1, submitted: -1, _id: -1},
	nextPath: function() {
		return Router.routes.bestDilemmas.path({dilemmasLimit: this.dilemmasLimit() + this.increment});
...
Router.route('/tag/:tagName?/:dilemmasLimit?', {name: "taggedDilemmas"});
...
DilemmaPageController = RouteController.extend({
	...
	data: function() {
		...
		//for rendering 404
		if(this.dilemmaWithUserDetailSub.ready && !this.dilemma())
		{
			return null;
		}
		else
